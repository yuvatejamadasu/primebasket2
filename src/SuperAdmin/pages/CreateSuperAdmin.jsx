...
import { UserPlus, User, Mail, Shield, CheckCircle2 } from 'lucide-react';
...
const CreateSuperAdmin = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setSuperAdmins(prev => [...prev, { ...formData, id: Date.now() }]);
    ...
    <div className="space-y-8 pb-8 animate-fade-in">
      ...
      {superAdmins.length > 0 && (
        <div className={`p-8 rounded-2xl border transition-all mt-8 ${isDark ? 'bg-[#2c3136] border-slate-700 shadow-xl' : 'bg-white border-slate-200 shadow-sm'}`}>
          <h3 className={`text-xl font-bold mb-4 ${isDark ? 'text-white' : 'text-slate-800'}`}>New Super Admins</h3>
