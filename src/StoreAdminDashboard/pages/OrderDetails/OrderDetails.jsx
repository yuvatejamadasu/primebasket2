          ...
          <div className="flex-1 overflow-y-auto max-h-[350px]">
            {pastOrders.length > 0 ? (
              <div className="p-4 space-y-3">
                {pastOrders.map(po => (
                  <div key={po.id} className={`p-4 rounded-xl border flex justify-between items-center transition-colors cursor-pointer ${isDark ? 'bg-slate-800/40 border-slate-700 hover:bg-slate-800/80' : 'bg-slate-50 border-slate-100 hover:bg-slate-100'}`} onClick={() => navigate(`/store-dashboard/orders/${po.id.replace('#', '')}`)}>
                    <div>
