...
import {
  ...
  CreditCard, User, Star, Award, BarChart2, Settings,
...
export const navItems = [
  ...
  { icon: <PlusCircle size={20} />, label: 'Add Product', path: '/store-dashboard/add-product' },
  { icon: <ShoppingCart size={20} />, label: 'Orders', path: '/store-dashboard/orders' },
  { icon: <Store size={20} />, label: 'Stores', path: '/store-dashboard/sellers' },
  { icon: <PlusCircle size={20} />, label: 'Create Admin', path: '/store-dashboard/create-admin' },
  ...
  { icon: <Award size={20} />, label: 'Brands', path: '/store-dashboard/brands' },
];
  ...
  return (
    <aside className={`sidebar fixed left-0 top-0 h-screen transition-all duration-300 z-50 border-r ${isMinimized ? 'w-20 collapsed' : 'w-64'
      ...
      <div className={`sidebar-header h-20 ${isMinimized ? 'px-0' : 'px-2'}`}>
        <Link to="/store-dashboard/dashboard" className="sidebar-brand">
      ...
      <nav className="px-3 space-y-1 overflow-y-auto h-[calc(100vh-160px)] custom-scrollbar">
        ...
        >
          <Settings size={20} />
          {!isMinimized && <span className="font-bold text-sm">Settings</span>}
          {isMinimized && <div className="absolute left-full ml-4 px-3 py-2 bg-slate-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 shadow-xl">Settings</div>}
        </NavLink>
      </nav>
    </aside>
  );
