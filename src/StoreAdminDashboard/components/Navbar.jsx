            ...
            {searchResults.brands.length > 0 && (
              <div className="p-2">
                ...
                {searchResults.brands.map(brand => (
                  <button 
                    key={brand.id}
                    onClick={() => handleNavigate(`/store-dashboard/brands/${brand.id}`)}
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-bold transition-all ${
                      isDark ? 'text-slate-200 hover:bg-slate-700/50 hover:text-brand-hover' : 'text-slate-700 hover:bg-slate-50 hover:text-brand'
                    }`}
                  >
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <div className="relative">
          <button 
            ...
            className={`relative p-2.5 rounded-lg transition-all group ${isDark ? 'text-slate-400 hover:bg-slate-800 hover:text-brand' : 'text-slate-500 hover:bg-slate-50 hover:text-brand'}`}
          >
            <Bell size={20} className="transition-transform group-hover:scale-110" />
            <span className="absolute top-2 right-2 w-4 h-4 bg-brand text-white text-[10px] font-bold rounded-full flex items-center justify-center border-2 border-white dark:border-[#1a1d21]">3</span>
          </button>
          
          {showNotifications && (
            <NotificationDropdown isDark={isDark} onClose={() => setShowNotifications(false)} />
          )}
        </div>
        ...
        <div className="relative" ref={profileRef}>
          ...
          {showProfileMenu && (
            <div className={`absolute right-0 mt-3 w-64 rounded-xl border shadow-2xl z-50 py-2 ${isDark ? 'bg-[#2c3136] border-slate-700' : 'bg-white border-slate-100'}`}>
              ...
              <DropdownItem icon={<User size={16} />} label="Edit Profile" isDark={isDark} onClick={() => handleNavigate('/store-dashboard/profile')} />
              <DropdownItem icon={<Settings size={16} />} label="Account Settings" isDark={isDark} onClick={() => handleNavigate('/store-dashboard/settings')} />
              <DropdownItem icon={<HelpCircle size={16} />} label="Help Center" isDark={isDark} onClick={() => handleNavigate('/store-dashboard/help')} />
              ...
              <DropdownItem icon={<LogOut size={16} />} label="Logout" isDark={isDark} danger onClick={() => {
                ...
                navigate('/store-dashboard/login');
              }} />
            </div>
          )}
        </div>
